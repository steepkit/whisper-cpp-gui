...
- SPA は fragment から token を取得後、即座に `history.replaceState` で URL から除去する。同一 tab の再読み込み回復に限り、検証済み token と active job ID を origin/tab 単位の `sessionStorage` に保存する。不正 fragment は除去して保存済みの有効 token へフォールバックする。再読み込み時は job の存在確認後に SSE snapshot へ再接続し、存在確認の一時エラーだけを上限付き backoff で再試行する。SSE error は header-authenticated API で確認し、401 なら token と job entry、job probe の 404 なら stale job entry を削除する。storage 上の無効値・通常 API の 401・存在しない job ID も該当 entry を削除し、storage 利用不能時は現在の page load だけで動作する。`localStorage` と Cookie には保存しない。応答に `Referrer-Policy: no-referrer` と `Cache-Control: no-store` を付ける
...
- 同一 tab の再読み込みで token と active job ID を復元し、job snapshot/SSE に再接続できる。不正 fragment からの fallback、一時エラーの上限付き retry、通常 API の 401、terminal job の SSE probe で受けた 401、job lookup と SSE 接続間の削除を再現する 404、storage 上の無効値・存在しない job ID の削除、`localStorage` / Cookie を使わないことを、実際の JavaScript を実行する browser behavior test で検証する
...
Scope:
- 本体リポジトリ: ubuntu-latest で `go vet` + `go test ./...`(スタブ E2E 含む) + Node.js built-in test runner/VM による browser behavior test
...
6. **UI E2E**: M2 では API/stub E2E、静的 i18n/DOM チェック、Node.js built-in test runner/VM で実 asset を実行する browser behavior test を優先する。npm package と bundler は導入しない。アクセシビリティツリー確認は必要なら開発・検証用手順として限定する
7. **CI**: ubuntu-latest で vet + test(E2E と browser behavior test を含む)、macos-14 でビルド + 起動スモーク。tap 側は `brew install --build-from-source` + `brew test`

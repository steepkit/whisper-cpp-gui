...
The active job ID is stored in the same session scope; after reload the SPA verifies that the job still exists and reconnects to its SSE snapshot. Transient lookup failures use a short bounded backoff. Invalid stored values and unauthorized ordinary API responses clear the corresponding session entries. An SSE error triggers a header-authenticated API probe: `401` clears the token and job entries, while a job-specific probe returning `404` clears the stale job entry even if the job disappeared between lookup and SSE subscription. If browser storage is unavailable, the current page remains usable with the fragment token but reload recovery is unavailable.
